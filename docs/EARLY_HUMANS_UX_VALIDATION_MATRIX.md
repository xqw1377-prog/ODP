...
Baseline: canonical `@odp/pilot` @ main `7b0cea0` (hackathon submission surface).
...
| ID | PRECONDITION | ACTION | EXPECTED | EVIDENCE | CURRENT STATUS (main @ 7b0cea0) | BLK SUBMIT | BLK RECRUIT |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
| 4 | tampered signature | submit enrollment | 400 "signature verification failed", nonce consumed | TEST + FIELD (wrong-signature probes) | PASS | NO | NO |
| 5 | same nonce reused | submit enrollment | 400 "challenge already used" | TEST + FIELD | PASS | NO | NO |
...
| 12 | reload page after enroll | re-open / | aggregate state persists; human record server-side | CODE + FIELD (local restarts) | PASS locally; **Fly persistence unverified** | NO | **YES (with 13)** |
...
| 16 | mobile Phantom in-app browser | full flow on phone | enroll + claim complete | UNTESTED | **OPEN** | NO | **YES** (significant mobile share expected) |
...
| 18 | Solflare instead of Phantom | connect via window.solana fallback | flow completes | UNTESTED | OPEN — documented path is Phantom; fallback expected via window.solana | NO | NO |
...
One cherry-pick (or branch merge) ports all three — **requires a gate ruling**;
not executed under the current freeze.
